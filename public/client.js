...
const lout = e => {
	e.target.classList.forEach(c => $(`.${c}`).removeClass("highlighted"));
};
...
const invert = e => {
	Object.keys(tables).map(k => tables[k]).forEach(r => r.selected = !r.selected);
	updateSelection();
};
...
function updateSelection() {
	...
	Object.keys(tables).filter(k => !tables[k].selected).forEach(k => $(`.multr${k}`).removeClass("selected"));
